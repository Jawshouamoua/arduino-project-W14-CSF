...
const unsigned int PING_I_PIN = 6 ;
const unsigned int PING_O_PIN = 7 ;

Oscil<CHUM9_NUM_CELLS, AUDIO_RATE> aCarrier(COS2048_DATA);
...
void setup(){
  pinMode(PING_O_PIN, OUTPUT) ;
  pinMode(PING_I_PIN, INPUT) ;
...
void updateControl(){
 ...
 // calculate the fm_intensity
  ...
  //set lfo speed based on SONAR reading
  digitalWrite(PING_O_PIN, LOW) ;
  delayMicroseconds(2) ;
  digitalWrite(PING_O_PIN, HIGH) ;
  delayMicroseconds(5) ;
  digitalWrite(PING_O_PIN, LOW) ;
  
  duration = pulseIn(PING_I_PIN, HIGH) ; //range of 116 - 23200
  if(duration > 5800) {
    duration = 5700 ;
  }
  Serial.println(duration) ;
  float mod_speed = (float)kMapModSpeed(duration) / 1000 ;
  */
